...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Public SMS verification requested for phone:", phone)
    ...
    console.log("[v0] Twilio Verify SID:", verifySid)
    console.log("[v0] Phone format (E.164):", phone)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: errorMessage, twilioError: data }, { status: 500 })
    }

    console.log("[v0] Verification SMS sent successfully to:", phone)
    console.log("[v0] Twilio response status:", data.status)

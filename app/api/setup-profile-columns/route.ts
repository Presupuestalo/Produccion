...
export async function POST(request: Request) {
  try {
    ...
    // SQL para a̱adir las columnas currency y tax_rate si no existen
    const sql = `
      -- Verificar si las columnas currency y tax_rate ya existen, si no, a̱adirlas
      DO $$
      BEGIN
        -- A̱adir columna currency si no existe
        IF NOT EXISTS (
    ...
    return NextResponse.json({ success: true, message: "Columnas a̱adidas correctamente" })
  } catch (error: any) {
